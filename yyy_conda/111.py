...
# 设置matplotlib全局参数
plt.rcParams['font.sans-serif'] = ['SimHei', 'Microsoft YaHei', 'SimSun', 'Arial Unicode MS', 'DejaVu Sans', 'sans-serif']  # 中文字体设置
...
# 要使用的特征列（中文列名初始加载）
FEATURE_COLS_CHINESE = ['标题长度', '作者数量', '作者h指数', '参考文献数量',
                       '五年影响因子', 'year_1', '篇幅', '十年CNCI']
...
def load_data(file_path):
    ...
    try:
        ...
        data['预测总引用量'] = data.iloc[:, 13:18].sum(axis=1) + data.iloc[:, 23:28].sum(axis=1)
...
def perform_shap_analysis(model, X_test, feature_names, y_pred_original):
    ...
    # 获取原始数据（未标准化）用于依赖图
    # 重新加载原始数据以确保获取中文列名
    original_file_path = "C:/Users/fangxiang/Downloads/GMM_processed/biology.xlsx"
    df_original = pd.read_excel(original_file_path)

    # 计算TNoC列（与load_data()中相同的计算）
    citation_cols = list(range(13, 18)) + list(range(23, 28))
    for col in citation_cols:
        df_original.iloc[:, col] = pd.to_numeric(df_original.iloc[:, col], errors='coerce').fillna(0)
    df_original['预测总引用量'] = df_original.iloc[:, 13:18].sum(axis=1) + df_original.iloc[:, 23:28].sum(axis=1)
    ...
    # 获取中文特征名
    chinese_feature_names = [reverse_mapping.get(col, col) for col in feature_names]

    # 确保feature_names与特征数量匹配
    if len(chinese_feature_names) != X_test.shape[1]:
        print(f"警告: 特征名称长度({len(chinese_feature_names)})与X_test列数({X_test.shape[1]})不匹配")
        chinese_feature_names = df_original.columns.tolist()
        print(f"改用数据框列名: {chinese_feature_names}")

    # 获取原始特征值（使用中文列名）
    X_original = df_original[chinese_feature_names].iloc[-len(X_test):]

    # 创建SHAP解释器
    explainer = shap.TreeExplainer(model)

    # [Rest of your existing SHAP analysis code remains the same...]
    # 计算SHAP值、生成可视化等代码保持不变
    ...
    try:
        ...
        if len(shap_values.shape) == 3:
            ...
            for class_idx in range(min(4, shap_values.shape[2])):
                ...
                plt.title(f'SHAP蜂窝图 - 类别{class_value}')
                plt.tight_layout()
                fix_negative_signs(plt.gcf())  # 修复负号显示
                plt.savefig(f'output/shap_beeswarm_class{class_value}.png', bbox_inches='tight')
                plt.close()
        else:
            plt.figure(figsize=(10, 8))
            ...
            for class_idx in range(min(4, shap_values.shape[2])):
                ...
                if class_idx in class_samples:
                    ...
                    try:
                        ...
                        plt.title(f'SHAP瀑布图 - 类别{class_value}')
                        plt.tight_layout()
                        fix_negative_signs(plt.gcf())  # 修复负号显示
                ...
                if class_idx in class_samples:
                    ...
                    if len(shap_values.shape) == 3:
                        ...
                        explanation = shap.Explanation(
                            values=sample_values,
                            base_values=base_value,
                            data=np.round(X_test.iloc[sample_idx].values, 3),  # 特征值也四舍五入
                            feature_names=feature_names
                        )
                        ...
                        plt.title(f'SHAP力图 - 类别{class_value}')
                        plt.tight_layout()
                        fix_negative_signs(plt.gcf())  # 修复负号显示
                        plt.savefig(f'output/shap_force_plot_class{class_value}.png', bbox_inches='tight', dpi=150)
                        plt.close()
            ...
            try:
                ...
                if len(shap_values.shape) == 3:
                    for class_idx in range(min(4, shap_values.shape[2])):
                        ...
                        plt.xlabel(feature + ' (原始值)')
                        plt.ylabel('SHAP值')
                        plt.title(f'SHAP依赖图 - {feature} - 类别{class_value}')
                        plt.tight_layout()
                        fix_negative_signs(plt.gcf())
                        plt.savefig(f'output/shap_dependence_{feature}_class{class_value}.png')
                        plt.close()
...
def main():
    ...
    # 1. 加载数据
    file_path = "C:/Users/fangxiang/Downloads/GMM_processed/biology.xlsx"
